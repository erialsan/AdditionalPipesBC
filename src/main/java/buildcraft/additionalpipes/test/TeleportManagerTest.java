...
public class TeleportManagerTest {
    ...
    public static void runAllTests() {
        // set the name of the current thread so that FMLCommonHandler.instance().getEfectiveSide returns the server
        // side.
    ...
    public static boolean testGetConnectedPipesSendReceive() {
        ...
        if (!(sendablePipes.size() == 1
            && (sendablePipes.contains(pipe2) && !(sendablePipes.contains(pipe1) && sendablePipes.contains(pipe3))))) {
            return false;
        }
        ...
        if (!(receivingPipes.size() == 1 && (receivingPipes.contains(pipe3)
            && !(receivingPipes.contains(pipe1) && receivingPipes.contains(pipe2))))) {
            return false;
        }
