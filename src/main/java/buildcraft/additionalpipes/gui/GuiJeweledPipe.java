...
public class GuiJeweledPipe extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        for (int tabNumber = 1; tabNumber <= NUM_TABS; ++tabNumber) {
            ...
            drawRect(
                ...
                tabEndX[tabNumber] - 1,
                tabY + tabHeight,
                tabColorsOutline[tabNumber - 1]);
            // draw inner rectangle
            drawRect(
                tabEndX[tabNumber - 1] + tabOutlineWidth,
                startY + tabOutlineWidth,
                tabEndX[tabNumber] - tabOutlineWidth - 1,
                tabY + tabHeight,
                tabColorsMain[tabNumber - 1]);
        }

        // add the tab labels
        for (int tabNumber = 1; tabNumber <= NUM_TABS; ++tabNumber) {
            String tabName = StatCollector.translateToLocal(
                "gui.tab." + ForgeDirection.VALID_DIRECTIONS[tabNumber - 1].toString()
                    .toLowerCase());
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        this.mc.getTextureManager()
            .bindTexture(Textures.GUI_JEWELED);

        this.drawTexturedModalRect(xStart, yStart, 0, 0, xSize, ySize);

        this.mc.getTextureManager()
            .bindTexture(Textures.GUI_OUTLINE_JEWELED[container.currentSide - 1]);
    ...
    public void initGui() {
        ...
        for (int tabNumber = 1; tabNumber <= NUM_TABS; ++tabNumber) {
            ...
            tabEndX[tabNumber] = tabEndX[tabNumber - 1] + fontRendererObj.getStringWidth(tabName)
                - 1
                + totalSpaceBetweenTabs;
        }

        // add the buttons
        buttonAcceptUnsorted = new GuiButtonOnOff(
            BUTTON_ID_UNSORTED,
            xStart + 8,
            yStart + 88,
            95,
        ...
        buttonMatchNBT = new GuiButtonOnOff(
            BUTTON_ID_NBT,
            xStart + 135,
            yStart + 88,
            30,
            container.pipeItemsJeweled.filterData[container.currentSide - 1].matchNBT(),
            StatCollector.translateToLocal("gui.NBT"));
        buttonMatchMetadata = new GuiButtonOnOff(
            BUTTON_ID_METADATA,
            xStart + 165,
            yStart + 88,
            30,
            container.pipeItemsJeweled.filterData[container.currentSide - 1].matchMetadata(),
            StatCollector.translateToLocal("gui.metadata"));
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        MessageJeweledPipeOptionsServer message = new MessageJeweledPipeOptionsServer(
            container.pipeItemsJeweled.container.xCoord,
            container.pipeItemsJeweled.container.yCoord,
            container.pipeItemsJeweled.container.zCoord,
            (byte) container.currentSide,
            currentSideFilter);
