...
public class PipeItemsAdvancedWood extends APPipe<PipeTransportAdvancedWood> implements IEnergyHandler {
    ...
    public ItemStack checkExtractGeneric(IInventory inventory, boolean doRemove, ForgeDirection from, int start,
        int stop) {
        for (int k = start; k <= stop; ++k) {
            ...
            if (slot != null && slot.stackSize > 0 && canExtract(slot)) {
                if (doRemove) {
                    int itemsExtracted = battery.getEnergyStored() / 10 >= slot.stackSize ? slot.stackSize
                        : MathHelper.floor_double(battery.getEnergyStored() / 10);
    ...
    public ItemStack checkExtractGeneric(ISidedInventory inventory, boolean doRemove, ForgeDirection from,
        ...
        for (int i : slots) {
            ...
            if (slot != null && slot.stackSize > 0
                && canExtract(slot)
                && inventory.canExtractItem(i, slot, from.ordinal())) {
                if (doRemove) {
                    int itemsExtracted = battery.getEnergyStored() / 10 >= slot.stackSize ? slot.stackSize
                        : MathHelper.floor_double(battery.getEnergyStored() / 10);
    ...
    public int getIconIndex(ForgeDirection direction) {
        if (direction == ForgeDirection.UNKNOWN) return 6;
        else {
            int metadata = container.getBlockMetadata();
            if (metadata == direction.ordinal()) return 7;
            else return 6;
        }
    }
