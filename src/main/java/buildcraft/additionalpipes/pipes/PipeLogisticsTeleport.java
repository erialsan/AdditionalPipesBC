...
public class PipeLogisticsTeleport extends PipeTeleport<PipeTransportItemsLogistics> {
    ...
    public void eventHandler(PipeEventItem.Entered event) {
        ...
        //
        // Position insertPoint = new Position(destination.xCoord + 0.5, destination.yCoord +
        // TransportUtils.getPipeFloorOf(event.item.getItemStack()), destination.zCoord + 0.5,
        // newOrientation.getOpposite());
        // insertPoint.moveForwards(0.5);
        // event.item.setPosition(insertPoint.x, insertPoint.y, insertPoint.z);
        //
    ...
    public PipeLogisticsTeleport getConnectedPipe() {
        List<PipeLogisticsTeleport> connectedPipes = TeleportManager.instance
            .<PipeLogisticsTeleport>getConnectedPipes(this, true, true);
        if (connectedPipes.size() == 0) {
            return null;
        } else if (connectedPipes.size() > 1) {
            Log.unexpected(
                "This Logistics Teleport Pipe has more than one other pipe on its channel.  Somewhere, somebody messed up!");
            return null;
        }
    ...
    public boolean blockActivated(EntityPlayer entityplayer, ForgeDirection direction) {
        Item equipped = entityplayer.getCurrentEquippedItem() != null ? entityplayer.getCurrentEquippedItem()
            .getItem() : null;
        if (equipped instanceof IToolWrench
            && ((IToolWrench) equipped).canWrench(entityplayer, container.xCoord, container.yCoord, container.zCoord)) {
