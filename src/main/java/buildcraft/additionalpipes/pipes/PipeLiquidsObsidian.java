...
public class PipeLiquidsObsidian extends APPipe<PipeTransportFluids> implements IEnergyHandler {
    ...
    private boolean suckItem(int distance) {
        ...
        List<EntityItem> discoveredEntities = container.getWorldObj()
    ...
    public void pullItemIntoPipe(EntityItem entity, int distance) {
        ...
        container.getWorldObj()
            .playSoundAtEntity(
                ...
                "random.pop",
                0.2F,
                ((container.getWorldObj().rand.nextFloat() - container.getWorldObj().rand.nextFloat()) * 0.7F + 1.0F)
                    * 2.0F);
        ...
        speed = Math
            .sqrt(entity.motionX * entity.motionX + entity.motionY * entity.motionY + entity.motionZ * entity.motionZ);
        speed = speed / 2F - 0.05;
    ...
    private void storeAndDrainItem(ItemStack stack) {
        ...
        // annoyingly, buckets don't use the standard Forge fluid API, so we need to use the separate
    ...
    private void dropCurrentItem() {
        ...
        entity.setPosition(
            entity.posX + direction.offsetX * 0.5d,
            entity.posY + direction.offsetY * 0.5d,
            entity.posZ + direction.offsetZ * 0.5d);

        entity.motionX = direction.offsetX * item.getSpeed() * 5 + getWorld().rand.nextGaussian() * 0.1d;
        entity.motionY = direction.offsetY * item.getSpeed() * 5 + getWorld().rand.nextGaussian() * 0.1d;
        entity.motionZ = direction.offsetZ * item.getSpeed() * 5 + getWorld().rand.nextGaussian() * 0.1d;
    ...
    public boolean canSuck(EntityItem item, int distance) {
        ...
        Item fluidItem = item.getEntityItem()
            .getItem();
        ...
        if (fluidItem instanceof IFluidContainerItem) {
            ...
            fluidID = FluidContainerRegistry.getFluidForFilledItem(item.getEntityItem())
                .getFluidID();
        }
