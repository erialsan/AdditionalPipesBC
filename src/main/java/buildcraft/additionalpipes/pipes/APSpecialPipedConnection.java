...
public class APSpecialPipedConnection implements ISpecialPipedConnection {
    ...
    EnumSet<PipeRoutingConnectionType> flags = EnumSet
        .<PipeRoutingConnectionType>of(PipeRoutingConnectionType.canRequestFrom, PipeRoutingConnectionType.canRouteTo);
    ...
    @Override
    public List<ConnectionInformation> getConnections(IPipeInformationProvider startPipeInfoGeneric,
        ...
        TileEntity pipeTile = startPipeInfoGeneric.getWorld()
            .getTileEntity(startPipeInfoGeneric.getX(), startPipeInfoGeneric.getY(), startPipeInfoGeneric.getZ());
            ...
            if (((IPipeTile) pipeTile).getPipe() instanceof PipeLogisticsTeleport) {
                ...
                if (farPipe != null) {
                    ConnectionInformation connectionInfo = new ConnectionInformation(
                        new PipeTeleportInformationProvider(farPipe),
                        flags,
                        nearPipe.getOpenOrientation()
                            .getOpposite(),
                        farPipe.getOpenOrientation()
                            .getOpposite(),
                        1);
